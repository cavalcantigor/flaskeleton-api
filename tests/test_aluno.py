...
def test_get_aluno_throw_error(client):
    with patch(
        "app.controllers.aluno.AlunoController.recuperar_aluno",
...
def test_post_aluno(client):
    ...
    response = client.post(
        "/flaskeleton-api/aluno/", json=data, headers=headers
    )
...
def test_post_aluno_throw_error(client):
    with patch(
        "app.controllers.aluno.AlunoController.criar_aluno",
        side_effect=Exception("Fake exception"),
    ):
        data = {
            "nome": "Jose Silva",
...
def test_post_aluno_exception(client):
    with patch(
        "app.controllers.aluno.AlunoDAO.insert",
        side_effect=Exception("Fake exception"),
    ):
        data = {
            "nome": "Jose Sousa",
...
def test_put_aluno_not_authorized_invalid_token(client):
    ...
    response = client.put(
        "/flaskeleton-api/aluno/1", json=data, headers={"Authorization": 456}
    )
    assert response.status_code == 401
    assert response.json["erro"] == "NAO_AUTORIZADO"
...
def test_put_aluno(client):
    ...
    response = client.put(
        "/flaskeleton-api/aluno/1", json=data, headers=headers
    )
    assert response.status_code == 200
...
def test_put_aluno_not_found(client):
    ...
    response = client.put(
        "/flaskeleton-api/aluno/100", json=data, headers=headers
    )
    assert response.status_code == 404
    assert response.json["erro"] == "NAO_ENCONTRADO"
...
def test_put_aluno_throw_error(client):
    with patch(
        "app.controllers.aluno.AlunoController.atualizar_aluno",
        side_effect=Exception("Fake exception"),
    ):
        data = {
            "nome": "Jose Silva",
...
def test_delete_aluno(client):
    assert (
        client.delete("/flaskeleton-api/aluno/1", headers=headers).status_code
        == 204
    )
    assert (
        client.get("/flaskeleton-api/aluno/1", headers=headers).status_code
        == 404
    )
...
def test_delete_aluno_throw_error(client):
    with patch(
        "app.controllers.aluno.AlunoController.deletar_aluno",
        side_effect=Exception("Fake exception"),
    ):
